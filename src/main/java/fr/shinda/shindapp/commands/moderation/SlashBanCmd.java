            ...
            if (event.getMember().getRoles().contains(event.getGuild().getRoleById(Ranks.OP.getRankId())) || authorData.getRank() >= Ranks.MOD.getId()) {
                ...
                event.getHook().sendMessageEmbeds(
                        new EmbedBuilder()
                                ...
                                .setDescription(String.format(Responses.GLOBAL_SANCTION_WAITING.getContent(), event.getMember().getUser().getName(), "Ban", reason))
                                .build()
                ).addActionRow(
                    ...
                    waiter.waitForEvent(ButtonClickEvent.class, e -> e.getMember().getId().equals(event.getMember().getId()) && e.getChannel().getId().equals(event.getChannel().getId()), e -> {
                        ...
                        if (e.getComponentId().equals(Buttons.BUTTON_SANCTION_CONFIRM.getButtonId())) {
                            ...
                            try {
                                ...
                                sanctionData.addBan();
                                sanctionData.setSanctionContent("Ban", reason, e.getMember());
                                user.ban(7).reason(String.format(Responses.GLOBAL_SANCTION_REASON.getContent(), e.getMember().getUser().getName(), "banni", user.getUser().getName(), reason)).queue();
                            } catch (Exception ex) {
                                ex.printStackTrace();
                            ...
                            e.getHook().sendMessageEmbeds(
                                    new EmbedBuilder()
                                            .setColor(Colors.MAIN.getHexCode())
                                            .setDescription(String.format(Responses.GLOBAL_SANCTION_REASON.getContent(), e.getMember().getUser().getName(), "banni", user.getUser().getName(), reason))
                                            .build()
                            ).queue();
                        }
